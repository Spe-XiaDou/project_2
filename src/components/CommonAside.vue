<template>
  ...
  >
    ...
    <!-- 没有子路由的部分 -->
    <el-menu-item @click="clickMenu(item)" v-for="item in noChildren" :index="String(item.path)" :key="item.path">
...
export default {
  ...
  data() {
    return {
      // isCollapse: false,
      menu: [{ path: '/home', name:'home', label: '首页', icon: 's-home', url: 'Home/Home'},
